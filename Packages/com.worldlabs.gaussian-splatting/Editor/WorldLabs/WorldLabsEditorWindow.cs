...
namespace WorldLabs.Editor
{
    ...
    public class WorldLabsEditorWindow : EditorWindow
    {
        ...
        private MarbleModel _model = MarbleModel.Standard;
        ...
        private void ShowImportMenu(WorldItem item)
        {
            ...
            // Mesh import option
            bool isMiniModel = string.Equals(item.World.model, "marble-1.0-draft", StringComparison.OrdinalIgnoreCase);
            if (isMiniModel)
            {
                menu.AddDisabledItem(new GUIContent("Mesh/Unavailable for marble-1.0-draft"));
            }
        ...
        private async void RefreshWorldsList()
        {
            ...
            try
            {
                MarbleModel? modelParam = _modelFilter == ModelFilter.All ? null : (MarbleModel?)(_modelFilter switch
                {
                    ModelFilter.Draft    => MarbleModel.Draft,
                    ModelFilter.Fast     => MarbleModel.Fast,
                    ModelFilter.High     => MarbleModel.High,
