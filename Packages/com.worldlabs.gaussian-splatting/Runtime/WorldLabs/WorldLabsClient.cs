...
namespace WorldLabs.API
{
    ...
    public class WorldLabsClient
    {
        ...
        /// <param name="model">The model to use (default: Standard / marble-1.1).</param>
        ...
        public async Task<GenerateWorldResponse> GenerateWorldFromTextAsync(
            ...
            MarbleModel model = MarbleModel.Standard,
            ...
            Action<GetOperationResponse> onProgress = null)
        {
            ...
            while (elapsed < timeoutSeconds)
            {
                ...
                if (response.error != null && (!string.IsNullOrEmpty(response.error.message) || (response.error.code != null && response.error.code != 0)))
                {
        ...
        /// <summary>
        /// Permanently deletes a world by ID. This cannot be undone.
        /// Returns a <see cref="DeleteWorldResponse"/> with <c>deleted == true</c> on success.
        /// Throws <see cref="WorldLabsException"/> with status 403 if you do not own the world,
        /// 404 if not found, or 422 for an invalid world_id format.
        /// </summary>
        /// <param name="worldId">The world identifier.</param>
        public async Task<DeleteWorldResponse> DeleteWorldAsync(string worldId)
        {
            return await DeleteAsync<DeleteWorldResponse>($"/marble/v1/worlds/{worldId}");
        }

        /// <summary>
        /// Permanently deletes a world by ID (coroutine version).
        /// Callback receives <c>null</c> on failure; check the Unity console for details.
        /// </summary>
        /// <param name="worldId">The world identifier.</param>
        /// <param name="callback">Invoked with the response on completion.</param>
        public IEnumerator DeleteWorld(string worldId, Action<DeleteWorldResponse> callback)
        {
            yield return DeleteCoroutine<DeleteWorldResponse>($"/marble/v1/worlds/{worldId}", callback);
        }
